...
static void gst_perf_set_property (GObject * object, guint prop_id,
...
display_current_fps (gpointer data)
{
  ...
  g_snprintf (fps_message, 255,
      "%s: frames: %" G_GUINT64_FORMAT " \tcurrent: %.2f\t average: %.2f", name,
...
gst_perf_class_init (GstperfClass * klass)
{
  ...
  trans_class->prepare_output_buffer =
      GST_DEBUG_FUNCPTR (gst_perf_prepare_output_buffer);
