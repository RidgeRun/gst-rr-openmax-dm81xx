...
gst_omx_scaler_set_caps (GstPad * pad, GstCaps * caps)
{
  ...
  if (!gst_structure_get_int (structure, "stride", &this->in_format.width_padded)) {
  ...
  if (!gst_structure_get_int (structure, "interlaced", &this->in_format.interlaced)) {
  ...
  gst_structure_fixate_field_nearest_int (srcstructure, "height",
      this->in_format.height);
  gst_structure_fixate_field_boolean (srcstructure, "interlaced", this->in_format.interlaced);
