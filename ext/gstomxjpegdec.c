...
/**
 * SECTION:element-omx_jpegdec
 *
 * Gstreamer JPEG video decoder using OpenMAX IL
 *
 * <refsect2>
 * <title>Example launch line</title>
 * |[
 * gst-launch -v -m fakesrc ! omx_jpegdec ! fakesink silent=TRUE
 * ]|
 * </refsect2>
 */
...
GST_DEBUG_CATEGORY_STATIC (gst_omx_jpeg_dec_debug);
#define GST_CAT_DEFAULT gst_omx_jpeg_dec_debug
...
static void
gst_omx_jpeg_dec_class_init (GstOmxJpegDecClass * klass)
{
  ...
  gst_element_class_set_details_simple (gstelement_class,
      "OpenMAX JPEG video decoder",
      "Codec/Decoder/Video",
      "RidgeRun's OMX based JPEG decoder",
  ...
  gstomxbase_class->init_ports =
      GST_DEBUG_FUNCPTR (gst_omx_jpeg_dec_init_pads);

  gstomxbase_class->handle_name = "OMX.TI.DUCATI.VIDDEC";

  /* debug category for fltering log messages */
  GST_DEBUG_CATEGORY_INIT (gst_omx_jpeg_dec_debug, "omx_jpegdec",
      0, "RidgeRun's OMX based JPEG decoder");
}
 ...
 */
static void
gst_omx_jpeg_dec_init (GstOmxJpegDec * this)
{
...
{
  GstOmxJpegDec *this = GST_OMX_JPEG_DEC (GST_OBJECT_PARENT (pad));
...
gst_omx_jpeg_dec_init_pads (GstOmxBase * base)
{
  GstOmxJpegDec *this = GST_OMX_JPEG_DEC (base);
...
gst_omx_jpeg_dec_fill_callback (GstOmxBase * base,
    OMX_BUFFERHEADERTYPE * outbuf)
{
  ...
  GST_LOG_OBJECT (this, "JPEG Fill buffer callback");
